...
__global__ void cc_loop3(double * __restrict rho, const double * __restrict  Vf,
...
__global__ void mc_loop3(double * __restrict rho, const double * __restrict  Vf,
						const double * __restrict x, const double * __restrict y, int sizex, int sizey, int Nmats) {
	int i = threadIdx.x + blockIdx.x * blockDim.x;
	int j = threadIdx.y + blockIdx.y * blockDim.y;
	if (i >= sizex || j >= sizey) return;
	...
	double *n, double *rho_ave)
{
  ...
  t1 = omp_get_wtime();
  cc_loop3<<<blocks, threads>>>(d_rho, d_Vf, d_x, d_y, sizex, sizey, Nmats);
  cudaDeviceSynchronize();
	...
	double *n, double *rho_ave)
{
  ...
  printf("Full matrix, material centric, alg 2: %g sec\n", omp_get_wtime()-t1);
	...
	// Computational loop 3 - Average density of each material over neighborhood of each cell
  t1 = omp_get_wtime();
  mc_loop3<<<blocks, threads>>>(d_rho, d_Vf, d_x, d_y, sizex, sizey, Nmats);
  cudaDeviceSynchronize();
  printf("Full matrix, material centric, alg 2: %g sec\n", omp_get_wtime()-t1);
